...
**Drift goes both ways, and only one direction is swept.** The function deletes files with no `asset_photos` row. The mirror image — a row whose file is gone — is the one a user actually sees, because Supabase signs a URL for a non-existent path without complaint, so `detail.js` can't filter it out and it renders as a broken image. `?report=true` is a read-only diagnostic listing both (`filesWithNoRow`, `rowsWithNoFile`); it takes no guards, since refusing to answer would hide the state someone is asking about. Nothing auto-fixes `rowsWithNoFile` — delete the row or re-upload the photo is a judgement call, not something a scheduled job should guess.
