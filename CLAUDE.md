...
- **Backend**: [Supabase](https://supabase.com) — Postgres for asset/repair/comment/profile records, Storage for photos, Auth for login. No custom server; every "backend" behavior lives in Postgres itself (RLS policies, triggers, CHECK constraints) or in the Supabase client SDK. The single exception is the scheduled Storage cleanup Edge Function — see "Storage cleanup" for why it can't be either, and why it isn't a precedent.
...
│           ├── index.ts                Deno entry point: lists, diffs, deletes
│           ├── orphans.js              Pure decision logic, unit tested from tests/
│           └── README.md               Why it exists, and how to deploy and schedule it
├── .github/
│   └── workflows/
...
**`schema.sql` describes the databases; it does not define them, and the two have drifted before.** The earliest tables predate `migrations/` — created by hand, with `schema.sql` written afterwards to describe them — so it declared an `on delete cascade` on `asset_repairs.asset_id` that **production never actually had**. Deleting an asset with repairs therefore failed in production, for months, on the app's only destructive action. Migration `0006` fixed it.

Two things follow, and both are the real lesson rather than the specific bug:
- **A green e2e suite says nothing about production's schema.** `register-view.spec.js` has always asserted that deleting an asset cascades its repairs, and has always passed — because the e2e project was created *later, from `schema.sql`*, and so had the cascade production lacked. The two databases are configured independently and only the e2e one is ever exercised by tests. Any schema assumption in this file is unverified against production until someone runs a query against production.
...
### Schema drift
`scripts/schema-report.sql` + `npm run schema:diff` exist because that assumption has now been wrong three times, every one of them in production, and not one of them caught by anything here:
...
- **#97 again (2026-08-08)** — `0007` created that missing policy, and photos *still* didn't arrive: production had **no Storage buckets at all**. `schema.sql` had described the bucket's access rules since the multi-photo work without ever declaring the bucket itself, and Postgres accepts a policy naming a `bucket_id` no bucket has, silently. Migration `0008`. Five real captures had been sitting in one phone's queue since they were taken, visible on that device and nowhere else; they synced within seconds of the bucket existing. **A fix that is written, reviewed and merged is not a fix that has been applied** — `migrations/` is applied by hand, and nothing anywhere reports which ones a given project has actually seen.
...
First run against real reports on 2026-08-08, long after it was written. It found four dead columns in production and nothing else: `assets.repair_needed`, `assets.repair_description`, `assets.repair_completed_at` and `asset_repairs.completed_comment` — the pre-`asset_repairs`/`repair_comments` design, referenced by no code and in no migration, harmless but worth dropping. Everything it exists to catch was clean by then. Run a day earlier it would have printed `bucket asset-photos: MISSING in production` in one round, instead of the afternoon that took by hand.
...
- **`asset_photos`** — up to four photos per asset, and the only place a photo path is recorded. `position` is a **sort key, not a count**: removing a photo leaves a gap rather than renumbering the rows that didn't change, because renumbering means extra writes and, offline, two devices renumbering the same set differently. New photos take `max(position) + 1`. The four-photo ceiling is enforced client-side only (`photoLimitReached` in `validation.js`) — under the shared "any logged-in user can edit everything" policy two people can each pass that check and produce a fifth row, the same class of problem as the last-write-wins limitation below, so the reading side tolerates it rather than the database forbidding it.
...
- **Orphaned Storage objects accumulate, and are swept periodically.** From partial failures above, and from `on delete cascade` dropping `asset_photos` rows when an asset is deleted without touching the files behind them. Neither is worth solving inline — see "Storage cleanup" below for the scheduled sweep that clears them.
- **Reading photos on the detail screen**: `detail.js` resolves them **once per asset view, not per render** — `drawView()` re-runs on every interaction, and re-signing four URLs each time would be a round trip for nothing. Signed URLs are fetched in one batch (`getPhotoUrls`), and queued-but-unsynced photos are shown from their **local blob** instead, since there's no object in Storage to sign yet; an asset captured offline still shows its photos. A photo that has just synced briefly exists in both (the row is written before the queue entry is dropped), so the saved copy wins on id and it isn't shown twice. A path that can't be signed is **kept and rendered as a "Photo unavailable" tile**, not dropped. `getPhotoUrls` returns `{ urls, unavailable }` for this reason. Dropping it was the old behaviour and it is what hid issue #97: a photo the database still has a row for looked identical to an asset that never had one, so the failure was unreportable. Offline is deliberately *not* reported this way — the timeout fallback returns an empty `unavailable` list, because that's a connection problem rather than a missing or forbidden object. **A failed reload keeps the last known saved photos rather than clearing them** — adding a photo offline re-runs this resolve, and with no connection the synced ones can neither be listed nor re-signed, so clearing would make photos already on screen vanish as a side effect of adding another. The signed URLs in hand stay valid for the hour they were issued for. The timeout fallback is therefore `null` (the marker for "didn't answer"), kept distinct from a query that genuinely returned no rows — which does mean the asset has no photos.
- **`assets` has no photo column.** A single `photo_path` lived there until migration `0005` dropped it, along with the dual-write that kept it current for clients running the pre-multi-photo build. Don't reintroduce one: an asset's photos are rows in `asset_photos`, and a denormalised "first photo" column is exactly the thing that goes stale on removal. Note for any future column removal here — dropping one is **not** a soft degradation for a stale service-worker-cached client: views name their columns explicitly in `.select()`, and PostgREST answers an unknown column with a 400, so the whole screen breaks rather than just the field. `registerType: 'autoUpdate'` means any client that opens the app online has already taken the new build, so the exposure is a device offline continuously since before the deploy — but stage the code change and the `drop column` across two deploys if that's ever a real risk.
...
**Storage has its own policies, entirely separate from the table policies above** — on `storage.objects`, not on any table in this schema. Getting the `assets` policy right does nothing for the bucket holding the images. Creating a bucket grants **no access at all** by default: RLS is on and, with no policy, every upload and every read is refused.

That gap caused issue #97. Production had RLS enabled on `storage.objects` with **zero policies**, so every photo upload had been failing with a 403 since the beginning — while `schema.sql` carried a comment asserting the policy "already existed on the production project". It never did; only the e2e project had it. Migration `0007` created the policy — and fixed nothing on its own, because production had no `asset-photos` bucket either; see `0008`.

Two things about how that presented are worth internalising, because they'll recur:
- **It looked like a visibility bug, not a total failure.** `syncQueuedPhotos()` catches a failed upload and leaves the photo queued — right for a dropped connection, indistinguishable from a 403 — and `detail.js` renders a queued photo from its **local blob**. So the person who took it saw it, on their device, indefinitely. Nobody else could, because it was never anywhere else. The one person positioned to notice was the one person who couldn't.
- **A single-account test can never see this**, and neither can the e2e suite: that project has always had the policy, so `asset-photos.spec.js` asserts photos reach Storage and passes while production is broken. `shared-register.spec.js` asserts the shared-register guarantees as a *second* account, which catches a regression in the intended design but still not drift between projects — that's issue #100.

The bucket must stay **private**. Making it public would make the symptom vanish by removing access control: every photo readable by anyone holding the URL, and the signed-URL mechanism in `supabase.js` pointless. `shared-register.spec.js` asserts an unauthenticated fetch is still refused, precisely to catch that over-correction.

Never put the Supabase *service role* key anywhere in this codebase — it bypasses RLS entirely and belongs only on a server this project doesn't have. Only the publishable (anon) key belongs in client code. Note this also means the cleanup Edge Function's successful bucket listing tells you **nothing** about user-level access; it bypasses policies.

## Storage cleanup
`supabase/functions/cleanup-orphaned-photos/` is a scheduled Edge Function that removes objects in the `asset-photos` bucket that no `asset_photos` row references. Its own README covers deploying, scheduling and the safety rails; what belongs here is why it exists at all and the one rule to keep.

**It is the only part of this project that runs outside the browser and outside Postgres**, and that's a deliberate exception, not a precedent. Deleting a Storage object needs the Storage API — removing the `storage.objects` row from SQL leaves the bytes in the backing store, which is the same orphan one level down. The `pg_cron` + `pg_net` alternative would have kept everything in the database but needs request bodies full of quoting, through exactly the paste-into-the-SQL-editor channel that has mangled quotes here repeatedly (see "Development environment"), plus a service-role credential in Vault. Don't move other backend behavior out here on the strength of this one; the rule in "Tech stack" still holds.

**The service role key lives only in that function's Supabase environment**, set through the dashboard or `supabase secrets set` — never in this repository, and never in a SQL statement anyone has to paste anywhere. A separate `CLEANUP_SECRET` authorises a call, so whatever schedules the sweep doesn't hold a credential that can touch the database at all.

**Drift goes both ways, and only one direction is swept.** The function deletes files with no `asset_photos` row. The mirror image — a row whose file is gone — it does not touch, and **neither direction is visible in the app**: `createSignedUrls` returns a per-path `error` with a null URL for a missing object, which `getPhotoUrls` and `detail.js` both filter out, so the asset shows "No photos yet." rather than a broken image. That's the reason `?report=true` exists. Silent drift is drift nobody will ever report, so the only way to know is to go and ask. It lists both (`filesWithNoRow`, `rowsWithNoFile`), deletes nothing, and takes no guards — refusing to answer would hide the state someone is asking about. Nothing auto-fixes `rowsWithNoFile`: delete the row or re-upload the photo is a judgement call, not something a scheduled job should guess.

**The decision logic is pure functions** (`orphans.js`, `selectOrphans` and `selectMissingObjects`) with no Deno, Supabase or network in it, unit tested in `tests/orphans.test.js` — the clock is passed in. `index.ts` does the listing, querying and deleting around it. Keep new rules on that side of the line: this is code that deletes files, and the test suite is the only thing standing between a logic change and a photo someone can't retake. It's plain `.js` rather than `.ts` precisely so Vitest and ESLint can read it without a TypeScript toolchain this project otherwise has no use for.

Three behaviors are load-bearing and easy to undo:

- **Deleting is opt-in — nothing is removed without `?apply=true`**, and the scheduled job has to pass it. It was the other way round for about an hour after first deployment, which was long enough to get caught: a `?report=true` call reached a deployment too old to know that parameter, which ignored it and fell straight through to a live destructive run. Nothing was lost, purely because there were no orphans at that moment. Destructive-by-default means every version skew, typo and forgotten flag fails *unsafe*; this way round the worst case is a report nobody asked for. Don't invert it back.
...
**It cannot tell an empty bucket from a bucket that isn't there.** Both read as `objects: 0`, and that is exactly what hid the missing bucket on 2026-08-08 — every report came back looking like a clean, empty register while uploads were failing. Listing a bucket that doesn't exist should be a loud error rather than a zero; until it is, confirm the bucket exists before reading anything into a zero.
...
7. Per-record sync failures are caught and left queued rather than thrown — a dropped connection mid-sync is an expected condition here, not a bug. `vite-plugin-pwa` separately caches the app shell (HTML/CSS/JS) so the app loads at all with no connection.
   - **The queue cannot tell "will sync later" from "will never sync", and that is a real limitation, not a detail.** A permanent rejection — a 403 from a missing Storage policy, a constraint violation — is caught by the same handler as a dropped connection and retried forever. Issue #97 lived in that blind spot for months: every photo upload was failing, the queue kept them, and nothing anywhere said so. `syncAll()` returns `{ succeeded, failed }` per entity; **every caller used to discard it**. When adding a photo, `detail.js` now checks that result and, *if online*, tells the user the upload failed. Offline stays silent, because that's what the queue is for. Any new sync call site should make the same distinction — online failure is worth surfacing, offline failure is not.
